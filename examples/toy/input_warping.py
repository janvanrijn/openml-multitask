...
def parse_args():
    ...
    parser.add_argument('--default_value_alpha', type=float, default=0.0)
    parser.add_argument('--default_value_beta', type=float, default=0.0)
...
def plot(x_train, y_train, x, params, plot_directory, target_name, param_name, num_samples=3):
    ...
    x = warp_input(x, params[2], params[3])
    x_train = warp_input(x_train, params[2], params[3])
...
def run(args):
    ...
    for y_idx in y_indices:
        ...
        plot(x, y, x_star, params, plot_directory=args.plot_directory, target_name=current_target, param_name=args.x_column)
